...
static void SetHeliRotorHealthHandler(const uint64_t nativeHash, void(*breakOffRotorFunc)(rage::fwEntity*))
{
	...
	fx::ScriptEngine::RegisterNativeHandler(nativeHash, [handler, breakOffRotorFunc](fx::ScriptContext& ctx)
	{
		...
		if (const auto vehicle = rage::fwScriptGuid::GetBaseFromGuid(vehicleHandle))
		{
			if (vehicle->IsOfType<CVehicle>())
			{
				const auto vehicleType = static_cast<eVehicleType>(*reinterpret_cast<int*>(reinterpret_cast<char*>(vehicle) + g_vehicleTypeOffset));
